...
then
    echo "Kustomize not found - installing to /usr/local/bin/..."
...
if [ -d "$MANIFESTS" ]; then
    ...
    echo "Cloning ${kubeflow_manifests} ${kubeflow_version} into $MANIFESTS"
...
# Deploy Kubeflow
while ! kustomize build $KUBEFLOW_KUSTOMIZE | envsubst '${CLUSTER_DOMAIN}'| awk '!/well-defined/' | oc apply -f -; do echo -e "Retrying to apply resources for Kubeflow..."; sleep 10; done
